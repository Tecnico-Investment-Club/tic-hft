...
OrdersManager::OrdersManager(const std::string& api_key,
    ...
    order_store_ = std::make_unique<OrderStore>(log_file);
    broker_ = std::make_unique<BinanceClient>(api_key, api_secret, dry_run);
    
    std::cout << "OrdersManager initialized" << std::endl;
}
...
void OrdersManager::submit_order(const Order& order) {
    ...
    order_store_->insert_order(order);
}
...
void OrdersManager::main_loop() {
    ...
    while (running_) {
        try {
            auto pending = order_store_->get_pending_orders();
            if (!pending.empty()) {
            ...
            auto active = order_store_->get_active_orders();
...
void OrdersManager::process_order(const Order& order) {
    try {
        if (dry_run_) {
            ...
            order_store_->update_order_status(order.order_id, OrderStatus::FILLED);
            return;
        }
        ...
        order_store_->update_order_binance_id(order.order_id, binance_order_id);
        order_store_->update_order_status(order.order_id, OrderStatus::NEW);
        ...
        order_store_->update_order_status(order.order_id, OrderStatus::REJECTED);
    }
}
...
void OrdersManager::update_order_status(const Order& order) {
    try {
        ...
        order_store_->update_order_status(order.order_id, new_status);
        ...
        if (response.contains("executedQty") && response.contains("cummulativeQuoteAssetTransactedQty")) {
            ...
            order_store_->update_order_fills(order.order_id, filled_qty, avg_price);
        }
...
void OrdersManager::log_event(uint64_t order_id, const std::string& message) {
